...
- 청산 중에도 손절은 포지션 0 확인까지 유지. 취소·조회가 실패하면 `CLOSING`을 유지하고
  다음 대사에서 다시 정리한다. 정리가 끝나야 `CLOSED` (이전 거래의 주문이
...
리포트 3일 이내. 종이매매는 현재 설정·계약 지문과 일치하고 실거래 시장의 공개 시세로
생성한 기록만 센다. 검증 기준 변경·미래/비정상 생성 시각·비정상 수치도 게이트를 닫는다.
