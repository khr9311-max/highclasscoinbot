"""
Naked Forex (Nekritin & Peters, 2012) 가격행동 규칙의 코인 이식.

책의 매매는 세 단계다 (5장 서두).
  1) 지지/저항 '존' 을 찾는다
  2) 가격이 존에 닿기를 기다린다
  3) 존 위에서 '촉매' 패턴이 찍히면 진입한다

이 모듈은 전부 순수 함수다(네트워크/디스크 없음). 실시간 판정(naked_strategy)과
백테스트(validate_naked)가 같은 코드를 쓰게 하기 위함이다. 판정은 항상
'i 번째 봉까지 마감된 데이터'만 본다 - 미래 봉을 참조하는 경로가 없어야
백테스트 결과를 믿을 수 있다.

책의 서술형 규칙을 숫자로 옮긴 부분은 각 함수 주석에 출처(장)와 함께 적었다.
임계값은 책에서 가져온 것이고 데이터로 맞춘 것이 아니다. 백테스트에서 이
값들을 바꿔가며 고르면 DSR/PBO 가 그만큼 나빠져야 정직하다.

업비트는 현물이라 공매도가 없다. 매수 패턴만 진입에 쓰고, 매도 패턴(약세
캥거루/빅섀도/물라)은 보유 포지션의 청산 신호로만 쓴다 (13장 'Using Market
Information' - 반대 신호가 존에서 찍히면 청산).

빅 벨트(9장)는 넣지 않았다. 외환의 '주말 갭' 을 전제로 한 패턴인데, 코인은
24시간 거래라 봉 사이에 갭이 생기지 않는다.
"""
