...
| 판단 | `multi_agent.py` | Gemini 점수 산출 + 결정론적 진입 규칙 |
...
| 점검 | `smoke_test.py` | 오프라인 통합 점검 (117개 항목) |
...
5. **[알려진 한계](#-알려진-한계) 절을 읽고 감수할 수 있는지 판단** —
   특히 손절이 없다는 점
6. **별도 계정 + 잃어도 되는 소액**으로 `DRY_RUN=false` 전환
7. 리스크 한도를 작게 시작해 점진 증액
8. 메타모델이 학습될 때까지(BUY/SELL 300건) 관찰. 학습 완료 시 텔레그램
...
- **손절/익절이 없다.** `place_market_sell` 을 호출하는 곳은 판정 규칙이
  SELL 을 낼 때 한 곳뿐이다. 진입가(`avg_buy_price`) 대비 손익으로 나가는
  로직이 없어서, 가격이 빠지는데 crypto 점수가 임계 미만이면 포지션이 그대로
  남는다. `DAILY_LOSS_LIMIT_KRW` 는 신규 매수만 막고 보유분을 강제 청산하지
  않는다. `triple_barrier.MetaLabeling` 은 `main.py` 에서 생성만 하고 실시간
  청산에 연결돼 있지 않다.
...
- **매도에도 60초 재주문 쿨다운이 적용된다.** 손절 타이밍을 늦출 수 있다.
