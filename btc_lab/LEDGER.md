...
## 월간 자동 재학습 (2026-09-26 추가, 같은 날 중지)

**중지:** 2026-09-26 13:40 UTC쯤 사용자 요청으로 서버의 `btc-ledger-retrain.timer`를 껐다(`systemctl disable --now`).
실거래 봇이 현물 전용이 되면서 시점 조정을 쓰지 않는다([현물 전용 전환](../btc_portfolio/deploy/AWS_SPOT_ONLY_2026-09-26.md)).
알트 학습 모델도 사전 검증을 통과하지 못했다([알트 검증](ALT_SEARCH_2026-09-26.md)).
원장은 마지막 모델로 기록과 텔레그램을 계속한다. `install-retrain.sh`를 다시 실행하면 타이머가 다시 켜지니 주의한다.
