"""Small BTCUSDT Spot REST boundary. No credentials are logged or represented.
...
class SpotGateway:
    def __init__(self, api_key="", api_secret="", allow_orders=False, transport=None):
    ...
    def __repr__(self):
        return f"SpotGateway(symbol={SYMBOL!r}, allow_orders={self.allow_orders!r})"
    ...
    async def market(self):
        ...
        if (spec.get("symbol"), spec.get("baseAsset"), spec.get("quoteAsset")) != (SYMBOL, "BTC", "USDT") or book.get("symbol") != SYMBOL:
        ...
        return {"server_time_ms": server_time, "bid": str(book["bidPrice"]), "ask": str(book["askPrice"]),
            "reference_price": str(reference), "filters": filters, "klines": klines, "symbol": SYMBOL,
            "base_asset": "BTC", "quote_asset": "USDT", "status": spec.get("status"),
    ...
    async def commission_rate(self):
        ...
        """
        result = _object(await self._request("GET", "/api/v3/account/commission", {"symbol": SYMBOL}, signed=True))
    ...
    async def relevant_filters(self):
        return _object(await self._request("GET", "/api/v3/myFilters", {"symbol": SYMBOL}, signed=True))
    ...
    async def open_orders(self, *, all_symbols=False):
        ...
        rows = await self._request("GET", "/api/v3/openOrders", {} if all_symbols else {"symbol": SYMBOL}, signed=True)
        if not isinstance(rows, list) or any(not isinstance(r, dict)
                or not isinstance(r.get("symbol"), str) or not r["symbol"]
                or (not all_symbols and r["symbol"] != SYMBOL) for r in rows):
    ...
    async def place_order(self, client_id, side, quantity, *, limit_price=None):
        ...
        """
        params = {"symbol": SYMBOL, "side": side, "type": "MARKET", "quantity": _quantity(quantity),
        ...
        if not isinstance(result, dict) or result.get("symbol") != SYMBOL or result.get("clientOrderId") != client_id:
            raise OrderOutcomeUnknown("Unexpected Spot order acknowledgement")
        return result

    async def get_order(self, client_id):
        try:
            result = await self._request("GET", "/api/v3/order", {"symbol": SYMBOL, "origClientOrderId": _client_id(client_id)}, signed=True)
        except GatewayError as error:
            if error.code == -2013:
                return None
            raise
        result = _object(result)
        if result.get("symbol") != SYMBOL or result.get("clientOrderId") != client_id:
    ...
    async def trades(self, order_id):
        ...
        for _ in range(MAX_TRADE_PAGES):
            rows = await self._request("GET", "/api/v3/myTrades", {"symbol": SYMBOL,
            ...
            for row in rows:
                ...
                if row.get("symbol") != SYMBOL or row.get("orderId") != order_id or trade_id <= previous:
