...
2026-09-25 구현 시점에 실제 주문을 제출하거나 AWS 거래 서비스를 전환하지 않았다.
...
[기존 백테스트와 실제 실행 규칙의 차이](BACKTEST_REVIEW_2026-09-25.md)를 전환 전에 확인한다.
...
COIN-M은 현재 교차(cross)·20배·원웨이였으며, 실행기의 격리·최대 3배 조건과 일치하지 않았다.
...
실체결·실이체·거래소 손절 발동·AWS 신규 서비스 가동은 아직 확인하지 않았다.
